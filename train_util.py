...
def poolfeat(input, prob, sp_h=2, sp_w=2):
    ...
    feat_ = torch.cat([input, torch.ones([b, 1, h, w]).cuda()],
                      dim=1)  # b* (n+1) *h*w
...
def assign2uint8(assign):
    ...
    red = torch.cat([torch.ones(size=assign.shape),
                     torch.zeros(size=[b, 2, h, w])], dim=1).cuda()
    ...
    mat_vis = torch.where(assign.type(torch.float) >= 0.,
                          red * (assign.type(torch.float) + 1) / 3, mat_vis)
    mat_vis = torch.where(assign.type(torch.float) >= 3.,
                          green * (assign.type(torch.float) - 2) / 3, mat_vis)
    mat_vis = torch.where(assign.type(torch.float) >= 6.,
                          blue * (assign.type(torch.float) - 5.) / 3, mat_vis)
...
def update_spixl_map(spixl_map_idx_in, assig_map_in):
    ...
    if (id_h == h) and (id_w == w):
        ...
        spixl_map_idx = F.interpolate(
            spixl_map_idx_in, size=(
                h, w), mode='nearest')

    assig_max, _ = torch.max(assig_map, dim=1, keepdim=True)
    assignment_ = torch.where(
        assig_map == assig_max, torch.ones(
            assig_map.shape).cuda(), torch.zeros(
            assig_map.shape).cuda())
    new_spixl_map_ = spixl_map_idx * assignment_  # winner take all
    new_spixl_map = torch.sum(
        new_spixl_map_,
        dim=1,
        keepdim=True).type(
        torch.int)
...
def get_spixel_image(given_img, spix_index, n_spixels=600,
    ...
    spixel_bd_image = mark_boundaries(
        ...
        color=(
            ...
            1,
            1))  # cyna
    return (cur_max * spixel_bd_image).astype(np.float32).transpose(2,
                                                                    0, 1), spix_index_np
