...
def make_dataset(dir):
    # we train and val seperately to tune the hyper-param and use all the data
    ...
    train_list = glob(os.path.join(dir,'train', '*_img.png'))
    val_list = glob(os.path.join(dir,'val', '*_img.png'))

    train_list = [os.path.relpath(p, dir) for p in train_list]
    val_list = [os.path.relpath(p, dir) for p in val_list]
...
def CITYSCAPES(root, transform=None, target_transform=None, val_transform=None,
    ...
    train_dataset = ListDataset(root, 'cityscapes', train_list, transform,
                                target_transform, co_transform,
                                loader=BSD_loader, datatype='train')

    val_dataset = ListDataset(root, 'cityscapes', val_list, val_transform,
